...
(function ($) {
    $.extend($.fn, {
        jTree: function (options) {
            ...
            return this.each(function () {
                ...
                $(">li", $this).each(function () {
                    ...
                    $li.genTree({
                        selfIcon: (typeof ($li.attr("iconCls")) == "undefined") ? '' : $li.attr('iconCls'), //自加，用于判断节点是否添加了iconCls
        ...
        subTree: function (op, level) {
            return this.each(function () {
                $(">li", this).each(function () {
                    ...
                    $this.genTree({
                        selfIcon: (typeof ($this.attr("iconCls")) == "undefined") ? 'file' : $this.attr('iconCls'), //自加属性，用于具有父亲节点的节点
                        icon: op.icon,
                        ckbox: op.ckbox,
        ...
        genTree: function (options) {
            var op = $.extend({ selfIcon: options.selfIcon, icon: options.icon, ckbox: options.ckbox, exp: "", coll: "", showSub: false, level: 0, options: null, isLast: false, filterButton: options.filter }, options);
            return this.each(function () {
                var node = $(this);
                var tree = $(">ul", node);
                ...
                if (tree.size() > 0) {
                    ...
                    if (op.icon) {
                        if (op.showSub) {
                            if (op.selfIcon != '') { cls = op.selfIcon; }
                            else { cls = op.options.folderColl; }
                        } else {
                            if (op.selfIcon != '') { cls = op.selfIcon; }
                            else { cls = op.options.folderExp }
                        }
                    }
                    $(">div", node).prepend("<div class='" + (op.showSub ? op.coll : op.exp) + "'></div>" + (op.ckbox ? "<div class='ckbox " + checked + "'></div>" : "") + (op.icon ? "<div class='" + cls + "'></div>" : ""));
                    $(">div>a", node).append(op.filterButton ? "<label id='filter_active' class='filter_active'></label><label id='filter_inactive' class='filter_inactive'></label>" : "");
                    ...
                    $(">div", node).prepend("<div class='node'></div>" + (op.ckbox ? "<div class='ckbox " + checked + "'></div>" : "") + (op.icon ? "<div class='" + cls + "'></div>" : ""));
                    $(">div>a", node).append(op.filterButton ? "<label id='filter_active' class='filter_active'></label><label id='filter_inactive' class='filter_inactive'></label>" : "");
                    addSpace(op.level, node);

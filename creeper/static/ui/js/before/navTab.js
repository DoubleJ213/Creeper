...
var navTab = {
	...
	_contextmenu:function($obj){ // navTab右键菜单
		...
		$obj.contextMenu('navTabCM', {
			bindings:{
				reload:function(t,m){
                    // Add by Zhu Weidong 2013-06-21
                    // BEGIN
                    var tabId = t.attr("tabid");
                    if(tabId == "Authorization Manage" || tabId == "机构管理"){
                        $this.clearProtreeAttr();
                    }
                    // END
	...
	_closeTab: function(index, openTabid){
        ...
        // monitor page clear timer task
        // BEGIN
        Timer.cancel(this._getTabs().eq(index).attr('tabId'));
        if (global_ws) {
            global_ws.close();
            global_ws = null;
        }
        // END
        // Add by Zhu Weidong 2013-06-14
        // BEGIN
        if(this._getTabs().eq(index).attr('tabId') == "Authorization Manage" || this._getTabs().eq(index).attr('tabId') == "机构管理"){
            this.clearProtreeAttr();
        }
        // END
	...
	closeAllTab: function(){
		var tmp = this._getTabs().filter(":gt(0)").remove();
        tmp.each(function(){
            Timer.cancel($(this).attr("tabId"));
            if (global_ws) {
                global_ws.close();
                global_ws = null;
            }
	...
	_closeOtherTab: function(index){
		...
		if (index > 0) {
			var str$ = ":eq("+index+")";
			var tmp = this._getTabs().not(str$).filter(":gt(0)").remove();
            tmp.each(function(){
                Timer.cancel($(this).attr("tabId"));
                if (global_ws) {
                    global_ws.close();
     ...
     * @param {Object} params: title, data, fresh
     */
    openTab_cover: function(tabid, url, options){ //if found tabid replace tab, else create a new tab.
        var op = $.extend({title:"New Tab", data:{}, fresh:true, external:false}, options);
     ...
     * 清除机构树中一些的隐藏属性
     */
    clearProtreeAttr:function(){
        if($("#project_tree_htmlVal")){
            $("#project_tree_htmlVal").val("");
        }
        if($("#project_target_url")){
            $("#project_target_url").val("");
        }

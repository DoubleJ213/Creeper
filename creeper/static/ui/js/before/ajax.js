 ...
 * @param {String} confirmMsg 提示确认信息
 */
function validateCallback2(form, callback, confirmMsg) {
    if ($("#progressBar")) {
    ...
    var _submitFn = function () {
        $.ajax({
            ...
            dataType:"json",
            cache:false,
            success:callback || DWZ.ajaxDone2,
            error:DWZ.ajaxError2
        });
    }
...
function dwzPageBreak(options) {
    ...
    if (op.rel) {
        ...
        if (form) {
            if (op.targetType == 'div') {
                $box = $("#" + op.rel);
                var th_widths = new Array();
                $box.children(" form ").eq(1).find(" .grid > .gridHeader > .gridThead > table > thead > tr > th").each(function () {
 ...
 * @param rel： 可选 用于局部刷新div id号
 */
function divPageBreak(args, rel) {
    dwzPageBreak({targetType:"div", rel:rel, data:args});
}

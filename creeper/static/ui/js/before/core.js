var DWZ = {
	...
	ajaxError:function(xhr, ajaxOptions, thrownError){
        if (xhr.status == 401) {
            ...
            window.location.href = "/authorize_manage/tips/"
        } else {
            if (alertMsg) {
                /**
    ...
    ajaxError_cover:function(xhr, ajaxOptions, thrownError){
    ...
    ajaxError2:function(xhr, ajaxOptions, thrownError){
        if (xhr.status == 401) {
            alertMsg.error(xhr.responseText);
        } else if (xhr.status == 403){
            window.location.href = "/authorize_manage/tips/"
...
(function($){
	...
	$.fn.extend({
        ...
        /**
         * @param {Object} op: {type:GET/POST, url:ajax请求地址, data:ajax请求参数列表, callback:回调函数 }
         * 打开navTab时  同时打开遮盖层&加载框
         */
        ajaxUrl_cover: function(op){
            var $this = $(this);


            /**
             * add by lishiquan 打开遮盖层&加载框
             */
            if($("#background")){
            ...
            $.ajax({
                ...
                success: function(response){
                    ...
                    /***
                     * add by lishiquan 响应成功之后关闭 遮盖层&加载框
                     */
                    if($("#background")){
                        $("#background").css("display","none");
                    }
		...
		layoutH: function($refBox){
			return this.each(function(){
                ...
                var overflow = $this.attr("overflow") || "auto";
				...
				if ($this.isTag("table")) {
                    ...
                    //////////////////////////////////////////////
                    ///// add by lishiquan 2012.9.4 获得Table所属DIV 应用于磁盘模块
                    /////////////////////////////////////////////
                    var siblings_H=0;
                    var div_ary=["volume_div","snapshot_div"];

                    var div_id = $this.parent().parent().parent().attr("id");
                    $this.parent().parent().parent().siblings().each(function(){
                        if($(this).attr("id")){
                            var volume_H = Math.ceil(iRefH/2);
                            var snapshot_H = Math.floor(iRefH/2);

                            if(navigator.userAgent.toLowerCase().indexOf("ie")!=-1){
                                $("#snapshot_div").height(Math.ceil(iRefH/2));
                                $("#volume_div").height(Math.floor(iRefH/2));
                            }

                            var tab = $("#"+div_id).parent().parent();

                            if(tab.css("display") == 'none'){
                                if(div_id == "snapshot_div"){
                                    siblings_H += volume_H;
                                }else if(div_id == "volume_div"){
                                    siblings_H += snapshot_H;
                                }
                            }else{
                                if(div_ary.indexOf($(this).attr("id"))!=-1){
                                    siblings_H += $(this).height();
                                }
                            }
                        }
                    });
                    $this.height(iH-siblings_H).css("overflow",overflow);

                    if(navigator.userAgent.toLowerCase().indexOf("ie")!=-1){
                        if($("#snapshot_div,#volume_div")){
                            $("#snapshot_div,#volume_div").css("height","50%");
                        }
                    }
				}
			});

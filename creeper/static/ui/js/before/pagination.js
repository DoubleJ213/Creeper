	...
	$.fn.extend({
		pagination: function(opts){
			...
			return this.each(function(){
                ...
                //update by zhaolei
                var current = pc.getCurrentPage();
                var start = interval.start;
                var end  = interval.end;
                if (end >= 4 && current < 4)
                {
                    start = interval.start;
                    if(end >= interval.start + 4)
                    {
                        end = interval.start + 4;
                    }
                }
                if(current >= 4 && current+1 != end)
                {
                    start = current-2
                    if(end >= current + 2)
                    {
				...
				$this.find(setting.nums$).each(function(i){
					_bindEvent($(this), i+start, pc.targetType(), pc.rel());
				});

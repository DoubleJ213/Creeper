...
#update by zhaolei api.base.is_service_enabled -> is_service_enabled
class NetworkClient(object):
    def __init__(self, request):
        from dashboard import api
        try:
            if is_service_enabled(request, 'network'):
                self.floating_ips = api.quantum.QuantumFloatingIpManager(request)
            else:
                self.floating_ips = api.nova.NovaFloatingIpManager(request)
        except Exception,e:
            raise e

...
from dashboard.utils.memoized import memoized

from dashboard.api.base import (APIResourceWrapper, QuotaSet,
...
class Server(APIResourceWrapper):
    ...
    _attrs = ['addresses', 'attrs', 'id', 'image', 'links',
             ...
             'OS-EXT-SRV-ATTR:host', 'console', 'umask']
...
class NovaFloatingIpManager(network.FloatingIpManager):
    ...
    def is_simple_associate_supported(self):
        return CONFIG["simple_ip_management"]
...
def server_terminate_spice_console(request, instance_id, console_type='spice-html5'):
    return novaclient(request).servers.terminate_spice_console(instance_id, console_type)


def server_gtk_console(request, instance_id, console_type='spice-client'):
    return SPICEConsole(novaclient(request).servers.get_gtk_console(
        instance_id, console_type)['console'])


def server_update_umask(request, instance_id, umask=0):
    return novaclient(request).servers.update_server_umask(instance_id, umask)
...
def server_change_pwd(request,instance_id,pwd):
    return Server(novaclient(request).servers.change_password(instance_id,pwd))
...
def server_backup(request,instance_id,backup_name, backup_type, rotation):
    return Server(novaclient(request).servers.backup(instance_id,backup_name,backup_type,rotation))
...
# add by wangqing, 2013.05.23
def server_instance_state(request, instance_id):
    """Get instance state of an instance."""
    return novaclient(request).servers.get_instance_state(instance_id)

def get_instance_network(request,instance_id):
#    return Server(novaclient(request).servers.get_ins_network(instance_id),request)
    return novaclient(request).servers.get_ins_network(instance_id)

def get_all_compute_nodes(request,instance_id):
    return novaclient(request).servers.get_compute_nodes(instance_id)
...
def server_live_migrate(request,server, host, block_migration=True, disk_over_commit=False):
    return novaclient(request).servers.live_migrate(server,host,block_migration,disk_over_commit)

def server_resize(request,instance_id,flavor_id):
    return novaclient(request).servers.resize(instance_id,flavor_id)

def server_confirm_resize(request, instance_id):
    return novaclient(request).servers.confirm_resize(instance_id)

def server_revert_resize(request, instance_id):
    novaclient(request).servers.revert_resize(instance_id)


def nova_tenant_quota_get(request, tenant_id):
    return QuotaSet(novaclient(request).quotas.get(tenant_id))


def nova_tenant_quota_update(request, tenant_id, **kwargs):
...
def instance_volumes_list(request, instance_id):
    from dashboard.api.cinder import cinderclient
...
def get_hypervisors_statistics(request):
    """
    v3.1.2 Add: get amounts of all compute nodes.
    """
    return novaclient(request).hypervisors.statistics()

def get_hypervisors_list(request):
    """
    v3.2.1 Add: get all of compute nodes list.
    """
